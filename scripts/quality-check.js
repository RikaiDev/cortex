...
function runCodeQualityChecks() {
  ...
  // TypeScript
  const tsResult = runCommand('npx tsc --noEmit --skipLibCheck', 'TypeScript compilation', true);
...
function runDependencyChecks() {
  ...
  for (const dep of criticalRuntimeDeps) {
    if (packageJson.devDependencies && packageJson.devDependencies[dep]) {
      print(RED, `❌ ERROR: "${dep}" is in devDependencies but used at runtime!`);
      allGood = false;
...
function runFutureDevelopmentChecks() {
  ...
  const grepResult = runCommand(grepCmd, 'Intentionally unused variables', true);
  ...
  const futureResult = runCommand(futureCmd, 'Future development variables', true);
