...
import type { Experience, MemoryIndex, MemorySearchResult, ExperienceIndexEntry } from '../types/memory.js';
...
export class MemoryService {
  ...
  async recordExperience(exp: Partial<Experience>): Promise<string> {
    ...
    const categoryPath = path.join(this.experiencesPath, `${experience.type}s`, fileName);
   ...
   */
  async searchExperiences(query: string, limit = 5): Promise<MemorySearchResult> {
    ...
    const scoredEntries = index.index.map((entry) => {
      ...
      for (const token of queryTokens) {
        if (titleLower === token) score += 2.0; // Exact match
      ...
      for (const tag of entry.tags) {
        ...
        for (const token of queryTokens) {
          if (tagLower === token) score += 1.5; // Exact tag match
  ...
  async enhanceContext(currentTask: string): Promise<string> {
    ...
    for (const exp of searchResult.experiences) {
      ...
      if (exp.successRate !== undefined) {
        contextParts.push(`**Success Rate**: ${(exp.successRate * 100).toFixed(0)}%`);
      }
      contextParts.push('');
  ...
  async extractLearnings(workflowId: string): Promise<void> {
    try {
      ...
      console.log(`Successfully extracted learnings from workflow: ${workflowId}`);
    } catch (error) {
      console.error(`Failed to extract learnings from workflow ${workflowId}:`, error);
    }
  }
   ...
   */
  private async analyzePhaseForLearnings(workflowId: string, phase: PhaseResult): Promise<void> {
    try {
      // Load phase content
      const content = await this.workflowService.loadPhaseContent(workflowId, phase.phase);
      ...
      console.error(`Failed to analyze phase ${phase.phase} for workflow ${workflowId}:`, error);
    }
  }
   ...
   */
  private extractPatterns(content: string, phase: PhaseResult): Array<{ title: string; content: string; tags: string[] }> {
    ...
    if (phase.validation.passed && phase.validation.messages) {
      for (const msg of phase.validation.messages) {
        if (msg.toLowerCase().includes('best practice') || msg.toLowerCase().includes('pattern')) {
          patterns.push({
   ...
   */
  private extractKeyDecisions(content: string, phase: PhaseResult): Array<{ title: string; content: string; tags: string[] }> {
      ...
      while ((phraseMatch = regex.exec(content)) !== null) {
        ...
        if (tech.length > 5 && tech.length < 100) {
          decisions.push({
            ...
            tags: [phase.phase, 'decision', 'technical', ...this.extractTagsFromText(tech)],
          });
        }
      }
    }
   ...
   */
  private extractSolutions(content: string, phase: PhaseResult): Array<{ title: string; content: string; tags: string[] }> {
   ...
   */
  private async detectLessonsLearned(workflowId: string, phases: PhaseResult[]): Promise<void> {
    ...
    for (const failed of failedPhases) {
      try {
        const content = await this.workflowService.loadPhaseContent(workflowId, failed.phase);
    ...
    for (const partial of partialPhases) {
      if (partial.validation.messages && partial.validation.messages.length > 0) {
        lessons.push({
  ...
  private extractTagsFromText(text: string): string[] {
    ...
    const techTerms = [
      'typescript', 'javascript', 'react', 'node', 'api', 'rest', 'graphql',
