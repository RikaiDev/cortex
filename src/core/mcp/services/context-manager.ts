...
export class ContextManager {
  ...
  private updateActiveTechnologies(
    ...
    if (techStack) {
      ...
      if (
        !context.activeTechnologies.some((tech) => tech.includes(techStack))
      ) {
        newTechs.push(entry);
      }
    }
    ...
    if (techInfo.database && techInfo.database !== "N/A") {
      ...
      if (
        !context.activeTechnologies.some((tech) =>
          tech.includes(techInfo.database!)
        )
      ) {
