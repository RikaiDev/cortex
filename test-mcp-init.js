...
async function testMCPInit() {
  ...
  console.log(`📁 .cortex exists before init: ${existsBefore ? '✅ Yes' : '❌ No'}`);
    ...
    initProcess.on('close', async (code) => {
      ...
      if (code === 0) {
        ...
        for (const check of checks) {
          ...
          console.log(`   ${check.name}: ${exists ? '✅ Created' : '❌ Missing'}`);
        ...
        if (await fs.pathExists(rolesDir)) {
          ...
          console.log(`   Role files copied: ${roleFiles.length > 0 ? '✅ Yes' : '❌ No'}`);
          if (roleFiles.length > 0) {
            console.log(`   📋 Role files: ${roleFiles.join(', ')}`);
            if (roleFiles.length >= 7) { // We expect 7 role files
              console.log('   All expected role files are present');
            } else {
              allPassed = false;
              console.log(`   ⚠️  Expected 7 role files, found ${roleFiles.length}`);
            }
          } else {
            allPassed = false;
          }
